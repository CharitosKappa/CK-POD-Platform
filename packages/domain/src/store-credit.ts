...
function validateAdjustment(
  ...
  return parseUsdCents(input.amount);
}

/** Caller must own the transaction; the account lock and ledger commit belong to that transaction. */
export async function adjustStoreCreditWithClient(
  client: SqlClient,
  actor: StoreCreditStaffActor,
  customerId: string,
  input: StoreCreditAdjustmentInput,
): Promise<StoreCreditAdjustmentResult> {
  const amountCents = validateAdjustment(actor, customerId, input);
...
export class StoreCreditService {
  ...
  async adjust(
    ...
    validateAdjustment(actor, customerId, input);
    return withTransaction(this.pool, (client) =>
      adjustStoreCreditWithClient(client, actor, customerId, input),
    );
  }
}
