...
The Creative canvas is the current hybrid review direction: its headline is intentionally set over two lines, its supporting sentence is a single line, and its composer overlaps the lower garment area. The visible prompt label is removed while its accessible label remains. Prototype controls allow CEO review of two inactive composer treatments: **Fade**, which visibly fades to transparent at the lower edge, and **Liquid Glass**, which uses a frosted translucent surface, light-catching rim, and backdrop blur. Both become an ordinary opaque composer with a complete border on focus so prompt entry stays clear.
